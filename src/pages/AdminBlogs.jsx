...
import { deleteAdminBlog, getAdminBlogs } from "../services/blog-admin.service";
function AdminBlogs() {
    ...
    useEffect(() => {
        // Keeps the admin blog table synchronized with search and pagination.
    ...
    return (<main className="min-h-screen bg-slate-100">
      <AppBar position="static" elevation={0} sx={{ bgcolor: "#0f172a" }}>
        <Toolbar className="mx-auto w-full max-w-7xl">
          ...
          <Button color="inherit" startIcon={<AddIcon />} onClick={() => navigate("/admin/blogs/create")}>Create</Button>
        </Toolbar>
      </AppBar>
      <section className="px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl space-y-6">
          <Card sx={{ p: 3, borderRadius: 3 }}>
            <TextField fullWidth label="Search blogs" value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }}/>
          </Card>
                    ...
                    <TableBody>
                      {items.map((item) => (<TableRow hover key={item.id}>
                          <TableCell>
                            ...
                            <Typography className="text-sm text-slate-500">/{item.slug}</Typography>
                          </TableCell>
                          <TableCell>{item.category || "General"}</TableCell>
                          <TableCell><Chip label={item.status || "published"} size="small"/></TableCell>
                          <TableCell align="right">
                            <IconButton onClick={() => navigate(`/admin/blogs/${item.id}/edit`)}><EditIcon /></IconButton>
